...
#============= - Blackburn-Sherris independent model - =====================
...
par_cov_BSi <- function(x0, delta, kappa, sigma, r, mu_bar, D_se=50){
  ...
  V_bar <- matrix(0, length(c(x0, delta, kappa, sigma, r)), length(c(x0, delta, kappa, sigma, r))) 
  theta_bar <- rep(0, length(c(x0, delta, kappa, sigma, r))) # - vector for the mean over the parameter estimates
  B_table <- matrix(NA, D_se, length(c(x0, delta, kappa, sigma, r))) # - Table for parameter storage
...
nLL_BSd_3F_serr_H <- function(vdParameters){
  ...
  # - Parameters
  x0 <- vdParameters[1:3]
  delta <- vdParameters[4:9]
  kappa <- vdParameters[10:12] # - take logs to ensure positivity
  dg_l_Sigma_chol <- vdParameters[13:15] # - log diagonal elements of the lower cholesky dec.
  odg_Sigma_chol <- vdParameters[16:18] # - off diagonal elements of the lower cholesky dec.
  r_c <- vdParameters[21]
  r_1 <- vdParameters[19]
  r_2 <- vdParameters[20]
...
par_cov_BSd_3F <- function(x0, delta, kappa, sigma_dg, Sigma_cov, r, mu_bar, D_se=50){
  ...
  ## - Step 2.2: Initialise V_bar, 
  V_bar <- matrix(0, length(c(x0, delta, kappa, sigma_dg, Sigma_cov, r)), length(c(x0, delta, kappa, sigma_dg, Sigma_cov, r))) 
  theta_bar <- rep(0, length(c(x0, delta, kappa, sigma_dg, Sigma_cov, r))) # - vector for the mean over the parameter estimates
  B_table <- matrix(NA, D_se, length(c(x0, delta, kappa, sigma_dg, Sigma_cov, r))) # - Table for parameter storage
  ...
  Jac <- matrix(0, 21, 21)
  diag(Jac[1:12, 1:12]) <- rep(1, 12) # the untransformed delta
...
nLL_AFNSd_serr_H <- function(vdParameters){
  ...
  x0 <- vdParameters[1:3]
  delta <- vdParameters[4]
  kappa <- vdParameters[5:7] # - take logs to ensure positivity
  dg_l_Sigma_chol <- vdParameters[8:10] # - log diagonal elements of the lower cholesky dec.
  odg_Sigma_chol <- vdParameters[11:13] # - off diagonal elements of the lower cholesky dec.
  r_c <- vdParameters[16]
  r_1 <- vdParameters[14]
  r_2 <- vdParameters[15]
...
co_asc_CIR_MI <- function(mu_bar, X_rnd, x0, delta, kappa, sigma, theta_Q, theta_P, r, max_iter=200, tol_lik=0.1){
  ...
  return(list(l_x0=l_x0_par, delta=delta_par, l_kappa=l_kappa_par, l_sigma=l_sigma_par, l_theta_Q=l_theta_Q_par, l_theta_P=l_theta_P_par, l_r=l_r_par))
}
...
par_cov_CIR <- function(x0, delta, kappa, sigma, theta_Q, theta_P, r, mu_bar, D_se=50){
  ...
  ## - Step 2.2: Initialise V_bar, 
  V_bar <- matrix(0, length(c(x0, delta, kappa, sigma, theta_Q, theta_P, r)), length(c(x0, delta, kappa, sigma, theta_Q, theta_P, r))) 
  theta_bar <- rep(0, length(c(x0, delta, kappa, sigma, theta_Q, theta_P, r))) # - vector for the mean over the parameter estimates
  B_table <- matrix(NA, D_se, length(c(x0, delta, kappa, sigma, theta_Q, theta_P, r))) # - Table for parameter storage
