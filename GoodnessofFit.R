...
residuals_std_AFGNSi <- function(observed, x0, delta1, delta2, kappa, sigma, r){
  ...
  residuals <- observed - mu_bar_hat_AFGNSi(x0, delta1, delta2, kappa, sigma, r, observed)
  H <- meas_err_BS(l_r1, l_r2, l_rc)
...
residuals_std_AFGNSd <- function(observed, x0, delta1, delta2, kappa, sigma_dg, Sigma_cov, r){
  ...
  residuals <- observed - mu_bar_hat_AFGNSd(x0, delta1, delta2, kappa, sigma_dg, Sigma_cov, r, observed)
  H <- meas_err_BS(l_r1, l_r2, l_rc)
  Var_y <- H + B_tT %*% R_matrix %*% t(B_tT)
...
residuals_std_CIR <- function(observed, x0, delta, kappa, sigma, theta_Q, theta_P, r){
  ...
  X_t <- KF_CIR_uKD(x0, delta, kappa, sigma, theta_Q, theta_P, r, observed)$X_t
