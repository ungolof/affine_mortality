...
CovEst_BS_BSi <- function(x0, delta, kappa, sigma, r, mu_bar, n_BS=500, t_ex = 4, max_it=200, tolerance_lev=0.1){
  ...
  for(i in 1:n_BS){
    ...
    x0_s <- as.numeric(rmvnorm(n = 1, mean=X_t_sm, sigma = S_t_sm))
...
CovEst_BS_BSd_2F <- function(x0, delta, kappa, sigma_dg, Sigma_cov, r, mu_bar, n_BS=500, t_ex = 4, max_it=200, tolerance_lev=0.1){
  n_ages <- nrow(mu_bar)
  n_years <- ncol(mu_bar)
...
CovEst_BS_BSd_3F <- function(x0, delta, kappa, sigma_dg, Sigma_cov, r, mu_bar, n_BS=500, t_ex = 4, max_it=200, tolerance_lev=0.1){
  n_ages <- nrow(mu_bar)
  n_years <- ncol(mu_bar)
...
CovEst_BS_GMki <- function(x0, delta, kappa, gamma, sigma, r, mu_bar, n_BS=500, t_ex = 4, max_it=200, tolerance_lev=0.1){
  par_table <- matrix(NA, n_BS, length(c(delta, kappa, sigma, r)))
  n_factors <- length(kappa)
  n_ages <- nrow(mu_bar)
  n_years <- ncol(mu_bar)
  ...
  for(i in 1:n_BS){
    ...
    #par_table_GMki[i,] <- optim(c(delta, kappa, log(sigma), log(r)), nLL_GMki_uKD_BS, mu_bar_BS=mu_bar_BS, gr = NULL, method = "BFGS", hessian = FALSE, control=list(maxit = 10000))$par # or coordinate ascent
...
CovEst_BS_AFNSi <- function(x0, delta, kappa, sigma, r, mu_bar, n_BS=500, t_ex = 4, max_it=200, tolerance_lev=0.1){
  par_table <- matrix(NA, n_BS, length(c(delta, kappa, sigma, r)))
...
CovEst_BS_AFNSd <- function(x0, delta, kappa, sigma_dg, Sigma_cov, r, mu_bar, n_BS=500, t_ex = 4, max_it=200, tolerance_lev=0.1){
  n_ages <- nrow(mu_bar)
  n_years <- ncol(mu_bar)
...
CovEst_BS_CIR <- function(x0, delta, kappa, sigma, theta_P, r, mu_bar, n_BS=500, t_ex = 4, max_it=200, tolerance_lev=0.1, wd=0){
  ...
  for(i in 1:n_BS){
    ...
    x0_s <- as.numeric(rtmvnorm(n=1, mu=X_t_sm, sigma=S_t_sm, lb=rep(1e-10, n_factors)))  #l_bound(rmvnorm(n = 1, mu=X_t_sm, Sigma = S_t_sm)))
